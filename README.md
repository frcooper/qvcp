...
ytxa D:\clips\2026-07 -Upgrade             # re-download the upgradable ones in place, tag the rest
```
...
| `SourceUrl`, `SourceUrlStatus` | The URL inside the comment sigil; `OK` when it names the same id, `Mismatch` when it names a different one, `Missing` when there is no sigil (a comment without the `[[SourceURL\|...]]` wrapper still counts as missing), `Added` when `-Upgrade` has just written one. mkv's upper-case `COMMENT` tag is read too. |
...
`-Upgrade` also repairs the files it does *not* re-download: one that is already the best resolution (`ResStatus: OK`) but has no `[[SourceURL|...]]` comment gets one, derived from its filename id. Since a bracketed id in a title is only a strong hint, the tag is written only once yt-dlp has resolved that id **and** the duration it reports agrees with the file's (within 5 s) — a live stream with no duration, or a file that is plainly a different length, is left alone with the reason in `Note`. A re-downloaded file gets its tag from yt-dlp anyway, `Mismatch` rows are left for you to judge, and `Unavailable` ones cannot be confirmed. The write is an `ffmpeg -c copy` remux into a `*.ytxa-new.<ext>` sibling — every stream and every other tag is carried over, any comment already there is kept behind the sigil, and the file's timestamps are preserved — which replaces the original only after the comment has been read back out of it. On success the row reads `SourceUrlStatus: Added` and the summary counts it under `SourceAdded`; on failure the original is untouched. `ffmpeg` must therefore be on `PATH` for `-Upgrade`.
